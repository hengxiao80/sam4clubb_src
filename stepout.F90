...
#ifdef UWM_STATS
use microphysics, only: nmicro_fields, mstor, micro_field
#endif
...
#ifdef PNNL_STATS
!MWSWong: THL budget
    qrain0(k)=sum(qpl(1:nx,1:ny,k))/float(nx*ny) !MWSWong:to convert HL to THL
    qice0(k)=sum(qci(1:nx,1:ny,k))/float(nx*ny) + sum(qpi(1:nx,1:ny,k))/float(nx*ny) !MWSWong:to convert HL to THL
#endif /*PNNL_STATS*/
  end do
  qtostor = q0 - qi0
#ifdef PNNL_STATS
  thelstor(1:nzm) = thel0(1:nzm)
#endif /*PNNL_STATS*/

!----

! set mstor to be the microphysical mixing ratios    
