...
#ifdef UWM_STATS
real   qpl0(nzm), qi0(nzm), qrain0(nzm), qice0(nzm), thel0(nzm), &  !MWSWong--added
       w2z0(nzm), w3z0(nzm) ! MWSWong-added
#endif /* UWM_STATS */
!----------------------------------------------------------------
...
real &
...
#ifdef UWM_STATS
    t2lerad(nz),t2leforc(nz),t2lebt(nz), & ! Added for enhanced budget terms.
#endif /*UWM_STATS*/
    q2leadv(nz),q2legrad(nz),q2lediff(nz),q2leprec(nz),q2lediss(nz), &
#ifdef UWM_STATS
    q2leforc(nz),q2lebt(nz), & ! Added for enhanced budget terms.
#endif /*UWM_STATS*/
    twleadv(nz),twlediff(nz),twlepres(nz),twlebuoy(nz),twleprec(nz), &
#ifdef UWM_STATS
    twlerad(nz),twleforc(nz),twlebt(nz), & ! Added for enhanced budget terms.
#endif /*UWM_STATS*/
    qwleadv(nz),qwlediff(nz),qwlepres(nz),qwlebuoy(nz),qwleprec(nz), &
#ifdef UWM_STATS
    qwleforc(nz),qwlebt(nz), & ! Added for enhanced budget terms.
#endif /*UWM_STATS*/
    ...
    thel2leadv(nz), thel2legrad(nz), thel2lediff(nz), thel2leprec(nz), thel2lediss(nz), &
    thel2lerad(nz), thel2leforc(nz), thel2lebt(nz), &
    thelwleadv(nz), thelwlediff(nz), thelwlepres(nz), thelwlebuoy(nz), thelwleprec(nz), &
    thelwlerad(nz), thelwleforc(nz), thelwlebt(nz), &
    ! MWSWong:QTOG budget "grand total water budget"
    qtog(dimx1_s:dimx2_s, dimy1_s:dimy2_s, nzm), &
    qtogadv(nz),qtogdiff(nz),qtogtend(nz),& 
    qtog2leadv(nz), qtog2legrad(nz), qtog2lediff(nz), qtog2leprec(nz), qtog2lediss(nz), &
    qtog2leforc(nz), qtog2lebt(nz), &
    qtogwleadv(nz), qtogwlediff(nz), qtogwlebuoy(nz), qtogwlepres(nz), qtogwleprec(nz), &
    qtogwleforc(nz), qtogwlebt(nz), &
    ! MWSWong:Q*THL budget
    qthelleadv(nz), qthelgrad(nz), qthellediss(nz), qthellebt(nz), qthelleprec(nz), &
    qthelleforc(nz), qtheltend(nz), qthellediff(nz), qthellerad(nz)
#endif /*PNNL_STATS*/
...
#if defined( PNNL_STATS ) || defined( UWM_STATS )
real   qtostor(nzm) ! Storage of horiz.-averaged total water profile (vapor + liquid)
real   qrstor(nzm) ! Storage of horiz.-averaged rain water profile
real   qistor(nzm) ! Storage of horiz.-averaged ice profile (qci+qs+qg)
real   thelstor(nzm)
#endif /*PNNL_STATS or UWM_STATS */
