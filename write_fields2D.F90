...
#ifdef CLUBB
! Vince Larson (UWM) allowed cloud water path and ice water path to be output
!   even when docloud = .false. and doclubb = .true.  12 Nov 2007
if( .not.docloud .and. .not.( doclubb.or.doclubbnoninter ) ) nfields = nfields-3
...
if(doshortwave) then
  ...
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
   ...
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=rtm_spurious_source(i,j)*coef
       pw_xy(i,j) = 0.
     end do
   end do
  name='RTMSPUR'
  long_name='Spurious moisture source'
  units='kg/m^2'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
   ...
   do j=1,ny
     do i=1,nx
       tmp(i,j,1)=thlm_spurious_source(i,j)*coef
       pw_xy(i,j) = 0.
     end do
   end do
  name='THLMSPUR'
  long_name='Spurious potential temperature source'
  units='(K kg)/m^2'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains)
