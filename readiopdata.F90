...
#ifdef UWM_MISC
  use grid, only: scamiop_from_global_cam
#endif
...
!CAM-IOP file, however it is all zero. For now, apply this fix to ignore this
!variable if using iop from global cam. weberjk(UWM)
   if( scamiop_from_global_cam ) then
     have_tsair = .false.
   endif
#endif
