...
use vars
#ifdef SILHS
use stat_clubb, only: stats_clubb_silhs_update
use clubb_silhs_vars, only: LH_microphys_disabled, LH_microphys_type
#endif
use rad, only: qrad
use tracers
use params
...
#ifdef PNNL_STATS
use microphysics, only: index_water_vapor, nmicro_fields, micro_field, flag_wmass
...
implicit none	
	...
	real swgrad(nzm)
#ifdef PNNL_STATS
	real thelwgrad(nzm) !MWSWong:convert HL to THL
        real qtogwgrad(nzm) !MWSWong: qv+qc+qi+qr+qs+qg
        real thel3z(nzm)    !MWSWong: third moment of theta_l
        real thel2z(nzm)    !MWSWong: variance of theta_l 
        real qtog2z(nzm)    !MWSWong: variance of "real" total water profile
...
real, dimension(nzm) :: rhowcl, rhowmsecl, rhowtlcl, rhowqtcl,  &
     ...
     rhowtvcl, rhowtvcla
#ifdef UWM_STATS
real, dimension(nzm) :: lhf_out, shf_out
real, dimension(nzm) :: vwp ! vapor water path [ kg / m^2 ]
#endif
...
#ifdef PNNL_STATS
!MWSWong: THL budget
	call hbuf_put('THLTEND',theltend,86400.)
	call hbuf_put('QTOGTEND',qtogtend,86400.*1.e3)
	call hbuf_put('QTHLTEND',qtheltend,1.)
#endif /*PNNL_STATS*/
...
!-------------------------------------------------------------
	...
	call hbuf_put('WSTAR3',wstar3,factor_xy) !bloss
        ...
        do j=1,ny
          do i=1,nx
            shf_out(1) = shf_out(1) + fluxbt(i,j)*cp*rhow(1)
            lhf_out(1) = lhf_out(1) + fluxbq(i,j)*lcond*rhow(1)
          end do
        end do
	call hbuf_put('SHFLX',shf_out,factor_xy)
	call hbuf_put('LHFLX',lhf_out,factor_xy)
#endif
!---------------------------------------------------------
...
!-----------------------------------------------------------
	...
	do k=1,nzm
	 ...
	 qs2z(k) = 0.
#ifdef PNNL_STATS
        thel3z(k) = 0. !MWSWong:added
        thel2z(k) = 0. !MWSWong:added
        qtog3z(k) = 0. !MWSWong:added
...
#ifdef PNNL_STATS
         if (.not.dompi) then !MWSWong: added if condition (see below for dompi)
#endif /*PNNL_STATS*/
...
#ifdef PNNL_STATS
	  thel2lediff(k)=thel2lediff(k)-thel2lediss(k)
	  qtog2lediff(k)=qtog2lediff(k)-qtog2lediss(k)
...
#ifdef UWM_STATS
	call hbuf_put('T2RAD',t2lerad,1.) ! Added for enhanced budget terms
	call hbuf_put('T2FORC',t2leforc,1.) ! Added for enhanced budget terms
	call hbuf_put('T2BT',t2lebt,1.) ! Added for enhanced budget terms
#endif /*UWM_STATS*/
#ifdef PNNL_STATS
	call hbuf_put('THL2ADVTR',thel2leadv,1.)
	call hbuf_put('THL2GRAD',thel2legrad,1.)
	call hbuf_put('THL2DISSIP',thel2lediss,1.)
	call hbuf_put('THL2DIFTR',thel2lediff,1.)
	call hbuf_put('THL2PREC',thel2leprec,1.)
	call hbuf_put('THL2RAD',thel2lerad,1.) ! Added for enhanced budget terms
	call hbuf_put('THL2FORC',thel2leforc,1.) ! Added for enhanced budget terms
	call hbuf_put('THL2BT',thel2lebt,1.) ! Added for enhanced budget terms
#endif /*PNNL_STATS*/
...
#ifdef UWM_STATS
	call hbuf_put('Q2FORC',q2leforc,1.) ! Added for enhanced budget terms
	call hbuf_put('Q2BT',q2lebt,1.) ! Added for enhanced budget terms
#endif /*UWM_STATS*/
#ifdef PNNL_STATS
        call hbuf_put('QTOG2ADVTR',qtog2leadv,1.)
        call hbuf_put('QTOG2GRAD',qtog2legrad,1.)
        call hbuf_put('QTOG2DISSIP',qtog2lediss,1.)
        call hbuf_put('QTOG2DIFTR',qtog2lediff,1.)
        call hbuf_put('QTOG2PREC',qtog2leprec,1.)
        call hbuf_put('QTOG2FORC',qtog2leforc,1.) ! Added for enhanced budget terms
        call hbuf_put('QTOG2BT',qtog2lebt,1.) ! Added for enhanced budget terms

!MWSWong new r'thl' budget terms
        call hbuf_put('QTHLADV',qthelleadv,1.)
        call hbuf_put('QTHLGRAD',qthelgrad,1.)
        call hbuf_put('QTHLDISSIP',qthellediss,1.)
	call hbuf_put('QTHLDIFTR',qthellediff,1.)
        call hbuf_put('QTHLBT',qthellebt,1.)
        call hbuf_put('QTHLPREC',qthelleprec,1.)
	call hbuf_put('QTHLFORC',qthelleforc,1.) ! Added for enhanced budget terms
	call hbuf_put('QTHLRAD',qthellerad,1.) ! Added for enhanced budget terms
#endif /*PNNL_STATS*/
	...
	do k=2,nzm
	 ...
	 do j=1,ny
	  do i=1,nx
	    fadv(k)=fadv(k)+w(i,j,k)**2*rhow(k)*0.5* &
#ifdef PNNL_STATS
       	     ( qv(i,j,k-1)+qcl(i,j,k)-(q0(k-1)-qi0(k-1))+qv(i,j,k)+qcl(i,j,k)-(q0(k)-qi0(k))) !MWSWong:qv+qc
#else
...
!  THLW advection d(w'w'thl')/dz:
        ...
        do k=1,nzm
         coef=-(fadv(k+1)-fadv(k))/(adz(k)*dz*rho(k))
         thelwgrad(k)=thelwleadv(k)-coef
         thelwleadv(k)=coef
        end do  



!  QTOGW advection d(w'w'qtog')/dz:
        ...
        do k=2,nzm
         ...
         do j=1,ny
          do i=1,nx
            fadv(k)=fadv(k)+w(i,j,k)**2*rhow(k)*0.5* &
                              ( qtog(i,j,k-1)-q0(k-1)-qp0(k-1)+qtog(i,j,k)-q0(k)-qp0(k))
          end do
         end do
        end do
        do k=1,nzm
         coef=-(fadv(k+1)-fadv(k))/(adz(k)*dz*rho(k))
         qtogwgrad(k)=qtogwleadv(k)-coef
         qtogwleadv(k)=coef
        end do  
...
!  Gradient terms for q'thl' (tp1+tp2)
        !do k=1,nzm
        !  qthelgrad(k)=0.
        !  do j=1,ny
        !    do i=1,nx
        !       qthelgrad(k) = qthelgrad(k)- w(i,j,k)*(qv(i,j,k)+qcl(i,j,k)-(q0(k)-qi0(k))) * &
        !              0.5*( (thel0(k+1)-thel0(k))/(adzw(k+1)*dz)+(thel0(k)-thel0(k-1))/(adzw(k)*dz)) &
        !            - w(i,j,k)*(thel(i,j,k)-thel0(k))*&
        !              0.5*( ((q0(k+1)-qi0(k+1))-(q0(k)-qi0(k)))/(adzw(k+1)*dz) + &
        !                    ((q0(k)-qi0(k))-(q0(k-1)-qi0(k-1)))/(adzw(k)*dz))
        !    end do
        !  end do
        !end do
...
#ifdef UWM_STATS
	call hbuf_put('TWRAD',twlerad,factor_xy) ! Added enhanced budget terms
	call hbuf_put('TWFORC',twleforc,factor_xy) ! Added enhanced budget terms
	call hbuf_put('TWBT',twlebt,factor_xy) ! Added enhanced budget terms
#endif /*UWM_STATS*/
#ifdef PNNL_STATS
!MWSWong new THL budget terms
        call hbuf_put('THLWADV',thelwleadv,factor_xy)
        call hbuf_put('THLWDIFF',thelwlediff,factor_xy)
        call hbuf_put('THLWGRAD',thelwgrad,factor_xy)
        call hbuf_put('THLWBUOY',thelwlebuoy,factor_xy)
        call hbuf_put('THLWPRES',thelwlepres,factor_xy)
        call hbuf_put('THLWPREC',thelwleprec,factor_xy)
        call hbuf_put('THLWRAD',thelwlerad,factor_xy) 
        call hbuf_put('THLWFORC',thelwleforc,factor_xy) 
        call hbuf_put('THLWBT',thelwlebt,factor_xy) 

!MWSWong new "grand" total water budget terms
        call hbuf_put('QTOGWADV',qtogwleadv,factor_xy)
	call hbuf_put('QTOGWDIFF',qtogwlediff,factor_xy)
	call hbuf_put('QTOGWGRAD',qtogwgrad,factor_xy)
	call hbuf_put('QTOGWBUOY',qtogwlebuoy,factor_xy)
	call hbuf_put('QTOGWPRES',qtogwlepres,factor_xy)
...
#ifdef UWM_STATS
	call hbuf_put('QWFORC',qwleforc,factor_xy) ! Added enhanced budget terms
	call hbuf_put('QWBT',qwlebt,factor_xy) ! Added enhanced budget terms
#endif /*UWM_STATS*/
...
#ifdef UWM_STATS
! Add vapor water path. pw_xy is already diagnosed in diagnose.f90
vwp(:) = 0.
do j=1,ny
  do i=1,nx
    vwp(1) = vwp(1) + pw_xy(i,j)   
  end do
end do
call hbuf_put('VWP',vwp,factor_xy)
#endif

#ifdef PNNL_STATS
      !MWSWong:convert HLRAD to THLRAD
      do k=1,nzm
         thelrad(k) = prespot(k) * prof1(k)    
      end do

      !theltend(1:nzm) = prespot(1:nzm)*ttend(1:nzm)  !see forcing.f90
#endif
...
#ifdef PNNL_STATS
!MWSWong: new THL budget
        call hbuf_put('THLADV',theladv,factor_xy*86400./dtn)
	call hbuf_put('THLDIFF',theldiff,factor_xy*86400./dtn)
	call hbuf_put('THLLAT',thellat,factor_xy*86400./dtn)
	call hbuf_put('THLRAD',thelrad,86400.)
#endif /*PNNL_STATS*/
...
#ifdef UWM_STATS
!+++mhwang storage terms are the instananeous value, but not accumulated over nstatis timestep.  
 ! This change makes it consistent with other tendency terms of u, v, t, and q.  
        ustor(:) = (u0(1:nzm) - ustor(:))/dtn 
        vstor(:) = (v0(1:nzm) - vstor(:))/dtn 
        tstor(:) = (t0(1:nzm) - tstor(:))/dtn 
        qstor(:) = (q0(1:nzm) - qstor(:))/dtn
!---MWSWong : below adapted from pnnl_branch_r1061 
        do k=1, nzm
          qi0(k)= sum(qci(1:nx,1:ny,k))/float(nx*ny)
        end do
        qtostor(:) = (q0(1:nzm)-qi0(1:nzm) - qtostor(:))/dtn
            call hbuf_put('QTOSTOR',qtostor,86400.*1.e3)
!---
#ifdef PNNL_STATS
       !MWSWong:convert HLSTOR to THLSTOR 
        do k=1,nzm
           qrain0(k)=sum(qpl(1:nx,1:ny,k))/float(nx*ny)
           qice0(k)=sum(qci(1:nx,1:ny,k))/float(nx*ny) + sum(qpi(1:nx,1:ny,k))/float(nx*ny) 
        end do
        thelstor(1:nzm) = (thel0(1:nzm)-thelstor(:))/dtn  
#endif /*PNNL_STATS*/
...
#ifdef PNNL_STATS
	call hbuf_put('THLSTOR',thelstor,86400.)
#endif /*PNNL_STATS*/
...
#ifdef PNNL_STATS
        !MWSWong: set up storage terms for next time
        qrstor(:) = qrain0(1:nzm)  ! = qr
        qistor(:) = qice0(1:nzm)   ! = qci+qs+qg 
        thelstor(:) = thel0(1:nzm) !***
#endif /*PNNL_STATS*/
! END UW ADDITIONS
!===================================================
