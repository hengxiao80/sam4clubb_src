...
use grid, only: &
  nx,ny,nzm,nz, &  !grid dimensions; nzm = nz-1 # of scalar lvls
...
real, allocatable, dimension(:) :: &
  mPRC, mPRA, mPSMLT, mEVPMS, mPRACS, mEVPMG, mPRACG, mPRE, mPGMLT, &
...
subroutine micro_setparm()
  ...
  ! maritime value (/cm3), adapted from Rasmussen 
  ! et al (2002) by Hugh Morrison et al.  Values
...
!bloss/qt: cloud liquid water no longer prognosed
  ...
  if(.not.isallocatedMICRO) then
    ...
    ! Microphysical process rates +++mhwang 
    allocate( &
      mPRC(nzm), mPRA(nzm), mPSMLT(nzm), mEVPMS(nzm), mPRACS(nzm), mEVPMG(nzm), mPRACG(nzm), mPRE(nzm), mPGMLT(nzm),&
...
subroutine micro_proc()
  ...
  use vars, only: &
    thel2leprec, thelwleprec, thel, thellat, &
  ...
  real, dimension(nzm) :: &
    PRC, PRA, PSMLT, EVPMS, PRACS, EVPMG, PRACG, PRE, PGMLT,  & 
...
subroutine micro_statistics()
  ...
  ! Morrison has indices for Nc, rr, etc. This is the offset
  ! between Morrison's and the correlation/covariance matrix indicies
  integer :: &
    idx_s, idx_w, idx_Nc, idx_rr, idx_Nr, idx_ri, idx_Ni,&
