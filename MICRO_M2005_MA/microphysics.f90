...
integer :: nfields2D_micro, nfields3D_micro
...
!bloss      do_micro3dout         ! flag to output 3d snapshot for a bunch of micro tendencies
  ...
  if(.not.isallocatedMICRO) then
    ...
    ! initialize flag arrays to all mass, no number, no precip
     ...
     if (doactivdiagoutput) then
        ! initialize a bunch of microphysical tendency 3D arrays
        NARG1 = 0.
        NARG2 = 0.
        NACTRATE = 0.
        QACTRATE = 0.
         ...
         do k = 1,nzm
           do j = 1,ny
             do i = 1,nx
               ...
               micro_field(i,j,k,iqad) = qa_accum(k)*massfactor ! dry aerosol mass
               micro_field(i,j,k,iqaw) = 0. ! wet aerosol mass

               micro_field(i,j,k,inad2) = Na_aitken(k)
               micro_field(i,j,k,iqad2) = qa_aitken(k)*massfactor
             end do
           end do
         end do
...
!         end do
       ...
       case(3)
         ...
         do k = 1,kinv
           ...
           micro_field(:,:,k,iqad2) = dumM2*massfactor
         end do
          
         do k = kinv+1, nzm
           micro_field(:,:,k,inad) = N_accum * aerinitFTfrac_acc
           micro_field(:,:,k,iqad) = dumM1*aerinitFTfrac_acc*massfactor
           micro_field(:,:,k,iqaw) = 0.         
           micro_field(:,:,k,inad2) = N_aitken * aerinitFTfrac_ait
           micro_field(:,:,k,iqad2) = dumM2*aerinitFTfrac_ait*massfactor
         end do
       ...
       do k = 1,kinv ! only initialize up to inversion height
         ...
         do m = 1,nx
           ...
           if( ( tmp_ind > nx_Nc1) .AND. (tmp_ind <= (nx_Nc1 + nx_trans)) ) then
             ...
             micro_field(m,:,k,iqad) = micro_field(m,:,k,iqad) + ( (-1.*dumM2)/2.*(cos(arg*pi)+1) + dumM2 )*massfactor
             ...
             micro_field(m,:,k,iqad) = micro_field(m,:,k,iqad) + dumM2*massfactor
             ...
             micro_field(m,:,k,iqad) = micro_field(m,:,k,iqad) + ( (-1.*dumM2)/2.*(cos(arg*pi+pi)+1) + dumM2 )*massfactor
           end if
...
if(doprogaerosol) then
  select case(aerfluxmode)
    ...
    flux_qad_xy(:,:) = flux_qad_xy(:,:) + fluxbmk(:,:,iqad)*dtfactor*RHOW(1)/massfactor ! units are kg/m2/s
    flux_qad2_xy(:,:) = flux_qad2_xy(:,:) + fluxbmk(:,:,iqad2)*dtfactor*RHOW(1)/massfactor
