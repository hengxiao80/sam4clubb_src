...
! SET MINIMUM AEROSOL NUMBER MIXING RATIO AND
!   AEROSOL PARTICLE MASS TO AVOID NUMERICAL
!   ISSUES WHEN DETERMINING PARTICLE SIZE
!   OR DUE TO UNDERSHOOTS RELATED TO ADVECTION.
         
! FIX MINIMUM PERMISSIBLE AEROSOL NUMBER
         NA_MIN = 1.E-3 ! #/KG

! FIX MINIMUM AEROSOL PARTICLE MASS AS MASS OF 3NM DIAMETER AEROSOL
         QA_PER_NA_MIN = (PI/6.)*RHOA*(3.E-9)**3 
...
! new variables for seifert and beheng warm rain scheme
      ...
      REAL :: NA_SHIFT, QA_SHIFT
      REAL :: NA_NEIGH, QA_NEIGH
      REAL :: QA_PER_NA_SHIFT
      INTEGER :: KLOW, KHIGH
...
!bloss(2023-10-23): Experiment with ensuring that aerosol number does not go to zero
            IF((NAD2_3D(K).LT.NA_MIN).OR. &
                 (QAD2_3D(K).LT.MAX(0.,NAD2_3D(K))*QA_PER_NA_MIN)) THEN

!!$               917 FORMAT('BEFORE SHIFT: NAK,QAK,RATIO,MIN_RATIO,QAC,NAC = ',6E12.4)
!!$               WRITE(*,917) NAD2_3D(K), QAD2_3D(K), QAD2_3D(K)/NAD2_3D(K), QA_PER_NA_MIN, NAD3D(K)+NC3D(K),QAD3D(K)
                    

               ! don't trust mean size for small aerosol mixing ratios
               !   Make sure aerosol is at least as big as 
               QA_PER_NA_SHIFT = QA_PER_NA_MIN

!!$               ! If we can get a valid (larger than 3 nm) aerosol size as
!!$               !   the mean among this and two neighboring cells, make the
!!$               !   Aitken mode that big.
!!$               KLOW = MAX(KTS, MIN(K-1,KTE-2) )
!!$               KHIGH = KLOW + 2
!!$
!!$               ! IF THERE IS ENOUGH AITKEN AEROSOL, TAKE THE NEW AITKEN SIZE
!!$               !   TO BE THE LARGER OF THE MIN VALUE (3NM) AND THE MEAN
!!$               !   SIZE OF THIS AND THE NEIGHBORING GRID CELLS.
!!$               NA_NEIGH = SUM(NAD2_3D(KLOW:KHIGH))
!!$               QA_NEIGH = SUM(QAD2_3D(KLOW:KHIGH))
!!$               
!!$               IF(NA_NEIGH.GT.3.*NA_MIN) THEN
!!$                  QA_PER_NA_SHIFT = MAX(QA_PER_NA_SHIFT, &
!!$                       QA_NEIGH/NA_NEIGH)
!!$               END IF
!!$                  
!!$               ! MAKE SURE THIS NEW AITKEN SIZE IS <= TO THAT
!!$               !   OF THE ACCUMULATION MODE AEROSOL
!!$               NA_NEIGH = SUM(NAD3D(KLOW:KHIGH)) + SUM(NC3D(KLOW:KHIGH))
!!$               QA_NEIGH = SUM(QAD3D(KLOW:KHIGH))
!!$               
!!$               IF(NA_NEIGH.GT.3.*NA_MIN) THEN
!!$                  QA_PER_NA_SHIFT = MIN(QA_PER_NA_SHIFT, &
!!$                       QA_NEIGH/NA_NEIGH)
!!$               END IF

               ! BORROW AEROSOL NUMBER/MASS FROM ACCUM MODE
               NA_SHIFT = MAX(0., NA_MIN - NAD2_3D(K) )
               QA_SHIFT = MAX(0., &
                    NA_MIN*QA_PER_NA_SHIFT - QAD2_3D(K) , & ! IF NAD2_3D(K) < NA_MIN
                    NAD2_3D(K)*QA_PER_NA_SHIFT - QAD2_3D(K) ) ! IF QAD2_3D < NAD2_3D(K)*QA_PER_NA_MIN

               ! ADD TO AITKEN MODE WITHOUT A CONSERVATION CONSTRAINT
               NAD2_3D(K) = NAD2_3D(K) + NA_SHIFT
               QAD2_3D(K) = QAD2_3D(K) + QA_SHIFT

!!$               NAD3D(K) = NAD3D(K) - NA_SHIFT
!!$               QAD3D(K) = QAD3D(K) - QA_SHIFT

               ! SAVE FOR STATISTICS/DIAGNOSTIC OUTPUT
               NAD2POSLIM(K) = NAD2POSLIM(K) + NA_SHIFT
               QAD2POSLIM(K) = QAD2POSLIM(K) + QA_SHIFT

!!$               NADNEGLIM(K) = NADNEGLIM(K) - NA_SHIFT
!!$               QADNEGLIM(K) = QADNEGLIM(K) - QA_SHIFT
!!$
!!$               IF(NAD3D(K).LT.0.) THEN
!!$                  ! IF THAT EXHAUSTED UNACTIVATED ACCUMULATION MODE AEROSOL,
!!$                  !   REMOVE OTHERS FROM CLOUD DROPLET NUMBER
!!$                  NC3D(K) = NC3D(K) + NAD3D(K)
!!$                  NAD3D(K) = 0.
!!$               END IF

!!$               918 FORMAT('AFTER SHIFT: NAK,QAK,RATIO,MIN_RATIO,QAC,NAC = ',6E12.4)
!!$               WRITE(*,918) NAD2_3D(K), QAD2_3D(K), QAD2_3D(K)/NAD2_3D(K), QA_PER_NA_MIN, NAD3D(K)+NC3D(K),QAD3D(K)
            END IF
               
            ! CHECK FOR BAD ACCUMULATION MODE NUMBER OR MASS VALUES
            IF((NAD3D(K)+NC3D(K).LT.NA_MIN).OR. &
                 (QAD3D(K)+QAW3D(K).LT.MAX(0.,NAD3D(K)+NC3D(K))*QA_PER_NA_MIN)) THEN

!!$               919 FORMAT('BEFORE SHIFT: NAC,QAC,RATIO,MIN_RATIO,QAK,NAK = ',6E12.4)
!!$               WRITE(*,919) NAD3D(K)+NC3D(K), QAD3D(K), QAD3D(K)/(NAD3D(K)+NC3D(K)), QA_PER_NA_MIN, NAD2_3D(K),QAD2_3D(K)
               ...
               NA_NEIGH = SUM(NAD3D(KLOW:KHIGH)) + SUM(NC3D(KLOW:KHIGH))
               QA_NEIGH = SUM(QAD3D(KLOW:KHIGH)) + SUM(QAW3D(KLOW:KHIGH))
               ...
               ! BORROW AEROSOL NUMBER/MASS FROM AITKEN MODE
               NA_SHIFT = MAX(0., NA_MIN - NAD3D(K) - NC3D(K) )
               QA_SHIFT = MAX(0., &
                    MAX(NA_MIN,NAD3D(K)+NC3D(K))*QA_PER_NA_SHIFT - QAD3D(K) - QAW3D(K) )

               ! ADD TO ACCUMULATION MODE WITHOUT CONSERVATION CONSTRAINT
               NAD3D(K) = NAD3D(K) + NA_SHIFT
               QAD3D(K) = QAD3D(K) + QA_SHIFT

!bloss               NAD2_3D(K) = NAD2_3D(K) - NA_SHIFT
!bloss               QAD2_3D(K) = QAD2_3D(K) - QA_SHIFT

!bloss               NC3D(K) = MAX(0., NC3D(K))

               ! SAVE FOR STATISTICS/DIAGNOSTIC OUTPUT
               NADPOSLIM(K) = NADPOSLIM(K) + NA_SHIFT
               QADPOSLIM(K) = QADPOSLIM(K) + QA_SHIFT

               !NOTE: THIS AEROSOL FIX IS NOT CONSERVATIVE
               !  BECAUSE I DO NOT WANT TO MESS UP THE AITKEN
               !  MODE SIZE.  CAN CHECK THE MISMATCH BETWEEN
               !  QADPOSLIM AND QAD2NEGLIM TO SEE HOW BIG
               !  THE MASS SOURCE IS.
               
!bloss               NAD2NEGLIM(K) = NAD2NEGLIM(K) - NA_SHIFT
!bloss               QAD2NEGLIM(K) = QAD2NEGLIM(K) - QA_SHIFT

!!$               920 FORMAT('AFTER ACCUM SHIFT: NAC,QAC,RATIO,MIN_RATIO,QAK,NAK = ',6E12.4)
!!$               WRITE(*,920) NAD3D(K)+NC3D(K), QAD3D(K), QAD3D(K)/(NAD3D(K)+NC3D(K)), QA_PER_NA_MIN, NAD2_3D(K),QAD2_3D(K)


!bloss               ! IF AITKEN AEROSOL IS BELOW MIN VALUES
!bloss               !   CREATE AEROSOL TO BRING IT UP TO MIN
!bloss               !   THIS VIOLATES CONSERVATION OF AEROSOL NUMBER/MASS
!bloss               !   BUT SHOULD HAPPEN RARELY AND GENERATE VERY SMALL
!bloss               !   AMOUNTS OF AEROSOL NUMBER/MASS.
!bloss               IF(NA_MIN.GT. NAD2_3D(K)) THEN
!bloss                  WRITE(*,*) 'CREATING ', NA_MIN - NAD2_3D(K), ' #/KG AITKEN AEROSOL NUMBER'
!bloss                  NAD2POSLIM(K) = NAD2POSLIM(K) &
!bloss                       + NA_MIN - NAD2_3D(K)
!bloss               END IF
!bloss               NAD2_3D(K) = MAX(NA_MIN, NAD2_3D(K))
!bloss
!bloss               IF(NAD2_3D(K)*QA_PER_NA_MIN.GT.QAD2_3D(K)) THEN
!bloss                  WRITE(*,*) 'CREATING ', NAD2_3D(K)*QA_PER_NA_MIN - QAD2_3D(K), ' KG/KG AITKEN AEROSOL MASS'
!bloss                  QAD2POSLIM(K) = QAD2POSLIM(K) &
!bloss                       + NAD2_3D(K)*QA_PER_NA_MIN - QAD2_3D(K)
!bloss
!bloss!!$921               FORMAT('AFTER AITKEN SOURCE: NAK,QAK,RATIO,MIN_RATIO,QAC,NAC = ',6E12.4)
!bloss!!$                  WRITE(*,921) NAD2_3D(K),QAD2_3D(K), QAD2_3D(K)/NAD2_3D(K), QA_PER_NA_MIN, NAD3D(K)+NC3D(K), QAD3D(K)
!bloss
!bloss               END IF
!bloss               QAD2_3D(K) = MAX(NAD2_3D(K)*QA_PER_NA_MIN, QAD2_3D(K))
