...
if ( do_spec_hum_to_mix_rat ) then
   ! The case specifications were given in terms of specific humidity.  They
   ! need to be put in terms of mixing ratio.  The moisture flux in terms of
   ! mixing ratio, <w'rt'>|_sfc, can be approximated by the equation:
   !
   ! <w'rt'>|_sfc = ( 1 + <rt> )^2 * <w'qt'>|_sfc;
   !
   ! where <w'qt'>|_sfc is the moisture flux in terms of specific humidity and
   ! <rt> is the grid-box mean value of total water mixing ratio.
   do i = 1, nx, 1
      do j = 1, ny, 1
         fluxbq(i,j) &
         = ( 1.0 + micro_field(i,j,1,index_water_vapor) )**2 * fluxbq(i,j)
      enddo ! j = 1, ny, 1
   enddo ! i = 1, nx, 1
