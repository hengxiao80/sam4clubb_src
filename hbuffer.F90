...
#ifdef UWM_MISC
write(*,*) 'filepath', trim(rundatadir)
#endif
...
trcount=0
#ifdef CLUBB
if(docloud.or.dosmoke.or.doclubb.or.doclubbnoninter) then
  call hbuf_micro_init(namelist,deflist,unitlist,status,average_type,count,trcount) ! UWM for CLUBB
end if
#else
...
! TODO: Fix this so count = huf_length, and duplicate entries are caught
! -dschanen 8 Sept 2014.
if ( LH_microphys_type /= LH_microphys_disabled ) then
  i = i+1
  namelist(i) = "LH_RT"
  deflist(i)  = "SILHS average total water (liquid + vapor) [kg/kg]"
  unitlist(i) = "kg/kg"
  status(i)       = 1
  average_type(i) = 0
  ...
  i = i+1
  namelist(i) = "LH_TL"
  deflist(i)  = "SILHS moist static energy [K]"
  unitlist(i) = "K"
  status(i)       = 1
  average_type(i) = 0
  ...
  i = i+1
  namelist(i) = "LH_S_MEL"
  deflist(i)  = "SILHS Mellor 's' 1977 [kg/kg]"
  unitlist(i) = "kg/kg"
  status(i)       = 1
  average_type(i) = 0
  ...
  i = i+1
  namelist(i) = "LH_W"
  deflist(i)  = "SILHS vertical velocity [m/s]"
  unitlist(i) = "m/s"
  status(i)       = 1
  average_type(i) = 0
  ...
  i = i+1
  namelist(i) = "LH_RR"
  deflist(i)  = "SILHS rain water mixing ratio [kg/kg]"
  unitlist(i) = "kg/kg"
  status(i)       = 1
  average_type(i) = 0
  ...
  i = i+1
  namelist(i) = "LH_RS"
  deflist(i)  = "SILHS snow water mixing ratio [kg/kg]"
  unitlist(i) = "kg/kg"
  status(i)       = 1
  average_type(i) = 0
  ...
  i = i+1
  namelist(i) = "LH_RI"
  deflist(i)  = "SILHS ice water mixing ratio [kg/kg]"
  unitlist(i) = "kg/kg"
  status(i)       = 1
  average_type(i) = 0
  ...
  i = i+1
  namelist(i) = "LH_NR"
  deflist(i)  = "SILHS rain number concentration [count/kg]"
  unitlist(i) = "count/kg"
  status(i)       = 1
  average_type(i) = 0
  ...
  i = i+1
  namelist(i) = "LH_NS"
  deflist(i)  = "SILHS snow number concentration [count/kg]"
  unitlist(i) = "count/kg"
  status(i)       = 1
  average_type(i) = 0
  ...
  i = i+1
  namelist(i) = "LH_NI"
  deflist(i)  = "SILHS ice number concentration [count/kg]"
  unitlist(i) = "count/kg"
  status(i)       = 1
  average_type(i) = 0
  ...
  i = i+1
  namelist(i) = "LH_NC"
  deflist(i)  = "SILHS cloud droplet number concentration [count/kg]"
  unitlist(i) = "count/kg"
  status(i)       = 1
  average_type(i) = 0
  ...
  i = i+1
  namelist(i) = "LH_TL_MC"
  deflist(i)  = "SILHS tendency of moist static energy [K/s]"
  unitlist(i) = "K/s"
  status(i)       = 1
  average_type(i) = 0
  ...
  i = i+1
  namelist(i) = "LH_RT_MC"
  deflist(i)  = "SILHS tendency of total water [kg/kg/s]"
  unitlist(i) = "kg/kg/s"
  status(i)       = 1
  average_type(i) = 0
  ...
  i = i+1
  namelist(i) = "LH_RR_MC"
  deflist(i)  = "SILHS tendency of rain water mixing ratio [kg/kg/s]"
  unitlist(i) = "kg/kg/s"
  status(i)       = 1
  average_type(i) = 0
  ...
  i = i+1
  namelist(i) = "LH_RS_MC"
  deflist(i)  = "SILHS tendency of snow water mixing ratio [kg/kg/s]"
  unitlist(i) = "kg/kg/s"
  status(i)       = 1
  average_type(i) = 0
  ...
  i = i+1
  namelist(i) = "LH_RI_MC"
  deflist(i)  = "SILHS tendency of ice water mixing ratio [kg/kg/s]"
  unitlist(i) = "kg/kg/s"
  status(i)       = 1
  average_type(i) = 0
  ...
  i = i+1
  namelist(i) = "LH_NI_MC"
  deflist(i)  = "SILHS tendency of ice number concentration [count/kg/s]"
  unitlist(i) = "count/kg/s"
  status(i)       = 1
  average_type(i) = 0

  i = i+1
  namelist(i) = "LH_NR_MC"
  deflist(i)  = "SILHS tendency of rain water number concentration [count/kg/s]"
  unitlist(i) = "count/kg/s"
  status(i)       = 1
  average_type(i) = 0
  ...
  i = i+1
  namelist(i) = "LH_NS_MC"
  deflist(i)  = "SILHS tendency of snow number concentration [count/kg/s]"
  unitlist(i) = "count/kg/s"
  status(i)       = 1
  average_type(i) = 0
...
#ifdef UWM_MISC  
                  status=filestatus,form='unformatted',iostat=stat)
  ! dschanen UWM added this and the iostat because he was tired of core dumps
  if ( stat /= 0 ) then
    if ( nrestart == 0 ) then
      write(0,*) "Stat file already exists, but this is not a restart run."
    else
      write(0,*) "Error opening stat file."
    end if
    call task_stop()
  end if
#else
...
#ifdef UWM_STATS
allocate ( corravg_count(hbuf_length,nzm) )
#endif /*UWM_STATS*/
call hbuf_flush()
...
#ifdef UWM_STATS
subroutine hbuf_put_level(name, f, factor, level)

!       Write to buffer for the given vertical level (not to file yet)
...
do  l = 1,hbuf_length
   if(.not.(lgt(name,namelist(l)).or.llt(name,namelist(l)))) then
       ...
       if(status(l).gt.0) then
          status(l) = 999
          begin = (l-1)*nzm
          hbuf(begin+level) = hbuf(begin+level) + f*factor
       endif
   endif
end do
if(.not.flag.and.masterproc) print*,'name ', name,' is missing in "lst" file.'

end subroutine hbuf_put_level
...
#ifdef UWM_STATS
    if ( average_type(l) == corr_avg ) then
       do k = 1, nzm, 1
          if ( corravg_count(l,k) > 0 ) then
             hbuf((l-1)*nzm+k) = hbuf((l-1)*nzm+k) &
                                 * ( float(n) / float( corravg_count(l,k) ) )
          else ! corravg_count(l,k) = 0
             hbuf((l-1)*nzm+k) = undef_corr
          endif
       enddo ! k = 1, nzm, 1
    endif ! average_type(l) == corr_avg
#endif /*UWM_STATS*/
  endif
end do
