...
#ifdef PNNL_STATS
! Note:  currently, these indices will not result in an accurate calculation for
!        SAM1MOM microphysics (theta-l/qtog budgets).
integer :: &
  ...
  indx_qs = 2, & ! Index for snow mixing ratio (theta-l/qtog adv. budgets)
  indx_qg = 2    ! Index for graupel mixing ratio (theta-l/qtog adv. budgets)
#endif /* PNNL_STATS */
...
#ifdef SILHS
real conc(nx, ny, nzm) ! This variable exists for compatibility
#endif
...
#ifdef CLUBB  
  use sgs_params, only: doclubb, doclubbnoninter ! dschanen UWM 21 May 2008
...
#if 0
#  /*!weberjk(UWM). Setting df = t just before the call to stat_varscalar, etc, 
#    !guarentees t2leprec and twleprec will be 0. Since the change in t due to
#    !microphysics is computed in micro_precip_fall, setting df = t before the call
#    !fixes this problem */
#endif
#ifndef UWM_STATS
...
#ifdef CLUBB
      if((docloud.or.doclubb).and.doprecip) then ! -dschanen 3 Jul 08
#else
...
subroutine micro_hbuf_init(namelist,deflist,unitlist,status,average_type,count,trcount)
  ...
  use vars
   ...
   do n = 1,nmicro_fields
      count = count + 1
      trcount = trcount + 1
      namelist(count) = TRIM(mkname(n))//'STO'
      deflist(count) = 'Storage term of '//TRIM(mklongname(n))
      unitlist(count) = TRIM(mkunits(n))//'day'
      status(count) = 1
      average_type(count) = 0
   end do
