...
#ifdef UWM_STATS
real presr(nz), qc0(nzm)!,qi0(nzm)  !MWSWong: qi0 set in vars.F90	
#else
 ...
 do iz = 2,n
  if(z(k).le.zz(iz)) then
    ...
    if ( do_spec_hum_to_mix_rat ) then
       ! The case specifications have been given in terms of specific humidity.
       ! Change specific humidity to mixing ratio.
       q0(k) = 1.0e-3 * q0(k)  ! Specify humidity from g/kg to kg/kg.
       q0(k) = q0(k) / ( 1.0 - q0(k) )  ! Specific humidity to mixing ratio.
       q0(k) = 1.0e3 * q0(k)   ! Mixing ratio from kg/kg to g/kg.
...
do k=1,nzm
  ...
  thel0(k) = prespot(k) * ( t0(k) - gamaz(k) ) !MWSWong:added thel0 
  qpl0(k) = 0. !MWSWong:added
#endif /*PNNL_STATS*/
  qv0(k) = q0(k)
  qc0(k) = 0.
  qi0(k) = 0.
...
#ifdef UWM_MISC /* Fix extropolation for 30 point grid */
...
#ifdef CLUBB
! dschanen (UWM) changed this for CLUBB 19 May 2008
if(docloud.or.dosmoke.or.doclubb.or.doclubbnoninter) call micro_init()  !initialize microphysics
#else
...
#ifdef UWM_STATS
! +++ mwhang (minghuai.wang@pnnl.gov) 
! store initial profiles for computation of storage terms in budgets 
  qstor = q0 
  tstor = t0  
  ustor = u0 
  vstor = v0
  qtostor = q0 - qi0 !---MWSWong: adapted from pnnl_branch_r1061
  thelstor = thel0 
#endif
if(masterproc) then
 print *,'Initial Sounding:'
 print *, ' k      z    rho     rhoi    s      h     h*      qt      u      v     adz     N'
#ifdef UWM_MISC /* dschanen found that a floating-point exception can happen at k=1 */
 do k=nzm,2,-1
#else
 do k=nzm,1,-1
#endif
