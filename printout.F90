	...
	use params
#ifdef CLUBB
        use sgs_params, only: doclubb, doclubb_sfc_fluxes, doclubbnoninter
#endif /* CLUBB */
	implicit none
...
#ifdef CLUBB
! Added to prevent redundancy - UWM
        if(doclubb.and.docloud) then
          print *, 'Both docloud and doclubb are set. Confused...'
          call task_abort()
        end if
        if(.not.doclubb.and.doclubb_sfc_fluxes) then
          print *, 'The flag doclubb_sfc_fluxes cannot be true when '// &
            'doclubb is false.'
          call task_abort()
        end if
#endif /*CLUBB*/
...
#ifdef CLUBB
        ! Vince Larson (UWM) generalized: precip is allowed when 
        !      docloud = .false. and doclubb = .true. 12 Nov 2007

        print*, 'precipitation is allowed:',doprecip.and. ( docloud .or. &
          doclubb.or.doclubbnoninter )
#else
